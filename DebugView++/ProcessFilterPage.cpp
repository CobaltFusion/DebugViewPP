...
void CProcessFilterPage::AddFilter(const ProcessFilter& filter)
{
	...
	m_grid.SetSubItem(item, 4, PropCreateColorItem(L"Background Color", filter.bgColor));
	m_grid.SetSubItem(item, 5, PropCreateColorItem(L"Text Color", filter.fgColor));
...
MatchType::type CProcessFilterPage::GetMatchType(int iItem) const
{
	CComVariant val;
...
std::vector<ProcessFilter> CProcessFilterPage::GetFilters() const
{
	...
	for (int i = 0; i < n; ++i)
		filters.push_back(ProcessFilter(Str(GetFilterText(i)), 0, GetMatchType(i), GetFilterType(i), GetFilterBgColor(i), GetFilterFgColor(i), GetFilterEnable(i)));
